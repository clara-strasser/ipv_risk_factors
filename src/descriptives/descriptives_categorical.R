...
## Variable "vio_inf_par"

# Calculate frequency counts
freq_table <- table(endireh_2021$vio_inf_par, useNA = "ifany")
...
# Plot
bar_chart_vio_inf_par <- ggplot(freq_df, aes(x = reorder(category, Freq_per), y = Freq_per, fill = category)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("no" = "#002b58", "yes" = "#620042", `NA` = "#4d565e")) +
  labs(x = "vio_inf_par", y = "Relative Frequency (%)") +
  ggtitle("Violence Witness Partner") +
  theme_minimal() +
...
ggsave(paste0(path_save, "bar_chart_vio_inf_par .png"), plot = bar_chart_vio_inf_par)

## Variable "vio_exp_inf_par"

# Calculate frequency counts
freq_table <- table(endireh_2021$vio_exp_inf_par, useNA = "ifany")
...
# Plot
bar_chart_vio_exp_inf_par <- ggplot(freq_df, aes(x = reorder(category, Freq_per), y = Freq_per, fill = category)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("no" = "#002b58", "yes" = "#620042", `NA` = "#4d565e")) +
  labs(x = "vio_exp_inf_par", y = "Relative Frequency (%)") +
  ggtitle("Violence Experience Partner") +
  theme_minimal() +
  theme(panel.grid = element_blank(),
        panel.border = element_blank(),
        axis.line = element_line(color = "black"),
...
# Save the ggplot as a PNG image
ggsave(paste0(path_save, "bar_chart_vio_exp_inf_par.png"), plot = bar_chart_vio_exp_inf_par)
...
bar_chart_vio_emo_año <- ggplot(freq_df, aes(x = reorder(category, Freq_per), y = Freq_per, fill = category)) +
  ...
  ggtitle("Emotional IPV Occurrence in the Last Year") +
  theme_minimal() +
  theme(panel.grid = element_blank(),
        panel.border = element_blank(),
        axis.line = element_line(color = "black"),
...
bar_chart_vio_emo_vida <- ggplot(freq_df, aes(x = reorder(category, Freq_per), y = Freq_per, fill = category)) +
  ...
  ggtitle("Emotional IPV Occurrence Since Begin of Marriage/Cohabitation") +
  theme_minimal() +
  theme(panel.grid = element_blank(),
        panel.border = element_blank(),
        axis.line = element_line(color = "black"),
